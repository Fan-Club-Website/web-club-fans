...
routes.use((err, req, res, next) => {
    console.log('err --> ', err);
    if (err instanceof moongose.Error.ValidationError) err = createError(400, err);
    else if (err.message?.includes('E11000')) err = createError(409, `${Object.keys(err.keyValue)[0]} already exist`);
